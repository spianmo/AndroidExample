...
class CourseActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tv_currentMouth.text = DateUtil.getMonthInEng() + DateUtil.getDate("dd")
        tv_currentWeek.text = currentWeek.toString() + "周"
        courseview.loadCourses(getData())
        courseview.setCurrentWeek(currentWeek)
        courseview.showAbsent(false)
        courseview.setCourseItemMargin(6)
        courseview.setOnCourseItemClickListener {
            it.toast(baseContext)
        }
        courseview.showDivider(false)
    }
